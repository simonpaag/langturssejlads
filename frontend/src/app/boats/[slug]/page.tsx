                                        ...
                                        {post.postType === 'ARTICLE' && (
                                            <div className="border border-border/50 rounded-2xl p-8 bg-muted/20 group-hover:bg-muted/40 transition-colors">
                                                <Link href={`/posts/${post.slug}`} className="block">
                                                    ...
                                                    {post.imageUrl && (
                                                        <div className="w-full aspect-video md:aspect-[21/9] bg-muted mb-6 border border-border/40 overflow-hidden rounded-xl relative shadow-inner">
                                                            <img src={post.imageUrl} alt={post.title || "Artikel Thumbnail"} className="absolute inset-0 w-full h-full object-cover hover:scale-105 transition-transform duration-1000 ease-out" />
                                                        </div>
                                                    )}
